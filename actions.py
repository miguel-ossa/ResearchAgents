...
import asyncio
from agents_ai import *
from config import *


BUSQUEDA_PRINCIPAL = "Últimos frameworks de Agentes de IA en 2025-2026"

async def ejecutar_busqueda():
    with trace("Búsqueda"):
        resultado = await Runner.run(agente_busqueda, BUSQUEDA_PRINCIPAL)
    return resultado

async def ejecutar_planificador():
    with trace("Planificador"):
        resultado = await Runner.run(agente_planificador, BUSQUEDA_PRINCIPAL)
    return resultado
